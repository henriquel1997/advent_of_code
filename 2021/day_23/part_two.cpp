...
bool can_move(State state, int amp_index){
	...
	if(a.room_index == a.type){
		if (a.position == 0){
			...
			bool has_other_below = false;
			bool has_same_above = false;
			for(int i = 0; i < MAX_AMPHIPODS; i++){
				if(i == amp_index) continue;
				Amphipod b = state.amphipods[i];
				if(a.room_index == b.room_index){
					if (a.type != b.type){
						has_other_below = b.position < a.position;
					} else {
						has_same_above = b.position > a.position;
					}
				}
			}
			return !has_same_above && has_other_below;
		}
	}
	
	if(a.position == ROOM_SIZE - 1) return true;
	
	for(int i = 0; i < MAX_AMPHIPODS; i++){
		if(i == amp_index) continue;
		Amphipod b = state.amphipods[i];
		if(b.room_index == a.room_index && b.position > a.position){
			return false;
		}
	}
...
int get_room_available_position(State state, int room_index){
	...
	for(int i = 0; i < MAX_AMPHIPODS; i++){
		...
		if(a.room_index == room_index){
			if(a.type != room_index || a.position == ROOM_SIZE - 1){
				return -1;
...
bool try_to_move_into_target_room(State* state, int amp_index){
	...
	if(room_pos >= 0){
		...
		if (is_hallway_path_clear(*state, a->position, room_entrance, amp_index)) {
			...
			state->energy_used += (dist + ROOM_SIZE - room_pos) * mult;
			a->room_index = a->type;
			a->position = room_pos;
	...
	for(int i = 0; i < MAX_AMPHIPODS; i++){
		if(can_move(state, i)){
			...
			if(a->room_index >= 0){
				...
				uint64 mult = get_energy_multiplier(a->type);
				new_state.energy_used += (ROOM_SIZE - a->position) * mult;
...
int main(){
	String text = readFile("input2.txt");
