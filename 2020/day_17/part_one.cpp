...
void printLayers(List* list){
	if (list->size <= 0) return;

	int64 minX = LLONG_MAX;
	int64 maxX = LLONG_MIN;
	int64 minY = LLONG_MAX;
	int64 maxY = LLONG_MIN;
	int64 minZ = LLONG_MAX;
	int64 maxZ = LLONG_MIN;
	...
	int64 layers = maxZ - minZ + 1;
	int64 width  = maxX - minX + 1;
	int64 height = maxY - minY + 1;

	int64 mapSize = width * height * layers;
	bool* map = (bool*) calloc(mapSize, sizeof(bool));

	for (ListNode* node = list->start; node; node = node->next) {
		Cube cube = node->cube;
		int64 index = ((cube.z - minZ) * (width * height)) + 
					  ((cube.y - minY) * (width)) + 
					  (cube.x - minX);
		map[index] = true;
	}
...
int main(){
	...
	List inactiveCubes = createList(MAX_NODES * 3);
